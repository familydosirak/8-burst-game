...
(() => {
  ...
  */
  ...
  // 블랙홀 흡입 및 폭발 설정
  const BLACK_HOLE_SUCTION_RADIUS = 90;
  ...
  // 구름공은 공을 관통하고 벽에서 두 번만 반사된다.
  const CLOUD_MAX_WALL_BOUNCES = 2;

  // 구름공이 직접 닿지 않아도 주변 공을 4로 바꾸는 판정 반경
  const CLOUD_CONVERT_RADIUS = BALL_RADIUS * 5;
  ...
  function specialBallDescription(type) {
    const descriptions = {
      rainbow:
        "처음 닿은 일반 숫자 공과 같은 색의 공을 맵에서 모두 제거합니다. 1에 닿으면 같은 빨간색인 1과 7이 모두 사라집니다. 폭발과 물리 충격은 발생하지 않습니다.",
